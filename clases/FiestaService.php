...
class FiestaService
{
	...
	*/
	public function __construct()
	{
		try
		{
			$this->conex = new PDO("mysql:host=SERVIDOR;dbname=NOMBRE BD", "USUARIO", "PASSWORD");
		}
	...
	*/
	public function Update($fiesta)
	{
		try
		{
			$sql = $this->conex->prepare("UPDATE Fiesta SET titulo = :titulo, descripcion = :descripcion, fecha = :fecha WHERE id = :id");
			$sql->bindParam('titulo',$fiesta->getTitulo());
			$sql->bindParam('descripcion',$fiesta->getDescripcion());
			$sql->bindParam('fecha',$fiesta->getFecha());
			$sql->bindParam('id',$fiesta->getId());
			$sql->execute();
			return 1;
		}
		catch(PDOException $e)
	...
	/**
	* @brief	Método que elimina una Fiesta identificada por id
	* @param	$id Identificador de la Fiesta a eliminar
	*/
	public function Delete($id)
	{
		try
		{
			$sql = $this->conex->prepare("DELETE FROM Fiesta WHERE id = :id");
			$sql->bindParam('id',$id);
			$sql->execute();
		}
		catch(PDOException $e)
